...
export default function CreateExhibitionsPage() {
  ...
  useEffect(() => {
    const fetchList = async () => {
      try {
        const resp = await listRequest(`v2/exhibitionsPage/?page=${page}&size=${size}`, 'GET');
        if (resp.data) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingExhibition) {
        await createRequest(`v2/exhibitions/${editingExhibition.id}`, 'PUT', form);
      } else {
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Вы уверены, что хотите удалить эту выставку?')) {
      try {
        ...
        await listRequest(`v2/exhibitionsPage/?page=${page}&size=${size}`, 'GET');
      } catch (e) {
  ...
  const getPageNumbers = () => {
    ...
    if (totalPages > max) {
      if (page <= 3) {
        ...
        start = totalPages - (max - 1); end = totalPages;
      } else {
  ...
  return (
    ...
    roleState && <main className="max-w-7xl mx-auto p-4">
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title={editingExhibition ? 'Редактировать выставку' : 'Создать выставку'} size="md">
        <form onSubmit={handleSubmit} className="space-y-4 p-4">
          ...
          {createError && <p className="text-red-500">Ошибка: {String(createError)}</p>}
        </form>
      </Modal>
      ...
      <div className="flex justify-center mt-6 space-x-1">
        ...
        {getPageNumbers().map(num => (
          <button
            ...
            className={`px-2 py-1 rounded ${num === page ? 'bg-black text-white' : 'border'}`}
          >
            {num}
          </button>
        ))}
